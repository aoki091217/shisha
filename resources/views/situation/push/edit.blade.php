...
@section('content')
{{ Breadcrumbs::render('situation.push.edit', $situation->id) }}
<form action="{{ route('situation.push.update', $situation->id) }}" method="POST" autocomplete="off" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    ...
    <div class="d-flex align-items-center justify-content-end mt-3 footer-buttons gap-2">
        <a href="{{ route('situation.push.show', $situation->id) }}" class="btn btn-secondary">戻る</a>
