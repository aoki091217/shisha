...
@section('content')
{{ Breadcrumbs::render('code.create') }}
<form action="{{ route('code.store') }}" method="POST" autocomplete="off">
    @csrf
    <div>
        ...
        <div class="col-8 mb-3">
            <select name="codes[shop_id]" id="shopName" class="form-select">
                ...
                @foreach ($shops as $shop)
                    <option value="{{ $shop->shop_id }}" {{ old('codes.shop_id') == $shop->shop_id ? 'selected' : '' }}>
                        {{ $shop->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-8 mb-3">
            <select name="codes[situation_id]" id="codeSituation" class="form-select">
                ...
                @foreach ($situations as $situation)
                    <option value="{{ $situation->id }}" {{ old('codes.situation_id') == $situation->id ? 'selected' : '' }}>
                        {{ $situation->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-8 mb-3">
            <textarea maxlength="5000" id="codeScript" name="codes[script]" class="form-control" style="height: 10rem;" data-name="text">{{ old('codes.script') }}</textarea>
            <span class="text-danger">{{ $errors->first('codes.script') }}</span>
        </div>
        <label for="codeNotes" class="form-label">備考欄</label>
        <div class="col-8 mb-3">
            <textarea maxlength="5000" id="codeNotes" name="codes[notes]" class="form-control" style="height: 10rem;" data-name="text">{{ old('codes.notes') }}</textarea>
